...
function getGreeting() {
    ...
    const words = [greeting, "ज्ञानम् एव मोक्षः"];
    let wordIndex = 0;
    let charIndex = 0;
    ...
    // Add keypress event listener for 'x' and 'X'
    document.addEventListener('keydown', function(event) {
        if (event.key === 'x' || event.key === 'X') {  // Check for both lowercase 'x' and uppercase 'X'
            window.location.href = '../codered/codered.html';  // Redirects to codered.html
        }
    });
});
